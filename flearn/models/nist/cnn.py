...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        images = tf.reshape(features, shape=[tf.shape(input=features)[0], 28, 28, 1])
        conv1 = tf.compat.v1.layers.conv2d(inputs=images, filters=32, kernel_size=3, activation=tf.nn.relu)
        conv2 = tf.compat.v1.layers.conv2d(inputs=conv1, filters=32, kernel_size=3, activation=tf.nn.relu)
        conv3 = tf.compat.v1.layers.conv2d(inputs=conv2, filters=32, kernel_size=5, strides=2, activation=tf.nn.relu)
        conv4 = tf.compat.v1.layers.conv2d(inputs=conv3, filters=64, kernel_size=3, activation=tf.nn.relu)
        conv5 = tf.compat.v1.layers.conv2d(inputs=conv4, filters=64, kernel_size=3, activation=tf.nn.relu)
        conv6 = tf.compat.v1.layers.conv2d(inputs=conv5, filters=64, kernel_size=5, strides=2, activation=tf.nn.relu)

        conv6_flatten = tf.compat.v1.layers.flatten(conv6)
        fc2 = tf.compat.v1.layers.dense(inputs=conv6_flatten, units=128, activation=tf.nn.relu, kernel_regularizer=tf.keras.regularizers.l2(0.001))
        logits = tf.compat.v1.layers.dense(inputs=fc2, units=self.num_classes, kernel_regularizer=tf.keras.regularizers.l2(0.001))
        predictions = {
