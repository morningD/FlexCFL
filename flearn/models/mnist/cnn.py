...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        conv1 = tf.compat.v1.layers.conv2d(inputs=images, filters=24, kernel_size=5, activation=tf.nn.relu)
        pool1 = tf.compat.v1.layers.max_pooling2d(inputs=conv1, pool_size=2, strides=2)
        pool1_flatten = tf.compat.v1.layers.flatten(pool1)
        fc2 = tf.compat.v1.layers.dense(inputs=pool1_flatten, units=256, activation=tf.nn.relu, kernel_regularizer=tf.keras.regularizers.l2(0.001))
